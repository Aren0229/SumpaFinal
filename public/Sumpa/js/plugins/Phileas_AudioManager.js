...
//-----------------------------------------------------------------------------
// Global playlist lock to prevent concurrent operations
AudioManager._playlistLock = false;
...
// API
    ...
    Game_System.prototype.launchPlaylist = function(files, random, loop, volume, pitch, pan) {
        // Prevent concurrent playlist operations
        if (AudioManager._playlistLock) {
            return;
        }
        
        AudioManager._playlistLock = true;

        // If a playlist is already active, stop it completely first
        if (this._playlist) {
            $gameSystem.stopPlaylist();
        }
        ...
        this._playlist = {
            ...
            _durationScheduled: false
        };

        // Use a small delay to ensure previous BGM is fully stopped
        setTimeout(() => {
            AudioManager._playlistLock = false;
            if (this._playlist) {
    ...
    Game_System.prototype.stopPlaylist = function() {
        ...
        if (this._playlist.timeout) {
            ...
            this._playlist.timeout = null;
            this._playlist._lastTimeoutId = null;
        }

        // Destroy any buffer that's being loaded
        if (this._playlist.currentBuffer) {
            try {
                this._playlist.currentBuffer.destroy();
            } catch (e) {
                // Ignore errors if buffer is already destroyed
            }
            this._playlist.currentBuffer = null;
        }

        // Destroy the currently playing buffer if it exists
        if (this._playlist.currentPlayingBuffer) {
            try {
                this._playlist.currentPlayingBuffer.destroy();
            } catch (e) {
                // Ignore errors if buffer is already destroyed
            }
            this._playlist.currentPlayingBuffer = null;
        }

        this._playlist.isLoading = false;
        this._playlist.isPlaying = false;
        this._playlist._durationScheduled = false;
        this._playlist = null;
        
        // Release lock after a brief delay to ensure cleanup
        setTimeout(() => {
            AudioManager._playlistLock = false;
        }, 50);
    };

    Game_System.prototype.skipToNextTrack = function() {
        if (!this._playlist) {
            return;
        }
        ...
        // Destroy any buffer that's being loaded (legacy, but keep for safety)
        if (this._playlist.currentBuffer) {
            try {
                this._playlist.currentBuffer.destroy();
            } catch (e) {
        ...
        // Also ensure AudioManager's buffer is destroyed
        AudioManager.stopBgm();
        ...
        // Wait a moment to ensure buffers are fully destroyed
        setTimeout(() => {
            if (this._playlist && !this._playlist.isLoading && !this._playlist.isPlaying) {
                $gameSystem.launchPlaylistTrack();
            }
        }, 150);
    };
...
// New code
    ...
    AudioManager.updateBgmParameters = function(bgm) {
        const modifier = ($gameSystem && $gameSystem._bgmModifier !== undefined) ? $gameSystem._bgmModifier : 1;
        this.updateBufferParameters(this._bgmBuffer, this._bgmVolume * modifier, bgm);
    };

    AudioManager.updateBgsParameters = function(bgs) {
        const modifier = ($gameSystem && $gameSystem._bgsModifier !== undefined) ? $gameSystem._bgsModifier : 1;
        this.updateBufferParameters(this._bgsBuffer, this._bgsVolume * modifier, bgs);
    };
    ...
    Game_System.prototype.launchPlaylistTrack = function() {
        ...
        // Prevent multiple tracks from loading or playing simultaneously
        if (this._playlist.isLoading || this._playlist.isPlaying) {
            return;
        }
        ...
        // Ensure any previous BGM is stopped before starting new track
        AudioManager.stopBgm();
        
        // Clear any previous playing buffer reference
        if (this._playlist.currentPlayingBuffer) {
            this._playlist.currentPlayingBuffer = null;
        }
        
        // Clear currentBuffer reference (not used anymore but keep for cleanup)
        this._playlist.currentBuffer = null;
        
        // Reset duration scheduling flag
        this._playlist._durationScheduled = false;

        this._playlist.isLoading = true;
        ...
        const playlist = this._playlist; // Store reference for closure
        const trackFileName = fileName; // Store filename for verification
        const currentTrackId = playlist.id - 1; // Store track ID for verification
        
        // Ensure any previous BGM is fully stopped
        AudioManager.stopBgm();
        
        // Wait a moment to ensure buffer is completely destroyed
        setTimeout(() => {
            // Double-check playlist is still valid and we're still on the same track
            if (!playlist || playlist.id - 1 !== currentTrackId || !playlist.isLoading) {
                if (playlist) {
            ...
            // Final safety check - ensure no BGM buffer exists
            ...
            function _playTrack() {
                ...
                // Mark as playing and stop loading
                playlist.isLoading = false;
                playlist.isPlaying = true;
                
                // Play the BGM (this creates its own buffer)
                AudioManager.playBgm(bgm);
                
                // Store the buffer reference to track it
                playlist.currentPlayingBuffer = AudioManager._bgmBuffer;
                
                // Check if timeout already scheduled (prevent double-scheduling)
                if (playlist._durationScheduled) {
                    return;
                }
                
                // Wait for the buffer to load to get duration
                if (playlist.currentPlayingBuffer) {
                    // Mark that we're scheduling duration to prevent duplicates
                    playlist._durationScheduled = true;
                    
                    // Function to schedule next track
                    const scheduleNext = () => {
                        // Verify we're still on the same track
                        if (!playlist || playlist.id - 1 !== currentTrackId || !playlist.isPlaying) {
                            playlist._durationScheduled = false;
                            return;
                        }
                        
                        // Get duration from the actual playing buffer
                        const duration = playlist.currentPlayingBuffer ? playlist.currentPlayingBuffer._totalTime : 0;
                        
                        if (duration) {
                            // Schedule next track after current one finishes
                            // Clear any existing timeout first
                            if (playlist.timeout) {
                                clearTimeout(playlist.timeout);
                                playlist.timeout = null;
                            }
                            
                            // Use a unique ID for this timeout to prevent duplicate callbacks
                            const timeoutId = Date.now() + Math.random();
                            playlist._lastTimeoutId = timeoutId;
                            
                            playlist.timeout = setTimeout(() => {
                                // Only proceed if this is still the current timeout
                                if (playlist._lastTimeoutId !== timeoutId) {
                                    return;
                                }
                                
                                // Only proceed if playlist is still active, playing, and not loading
                                if (playlist && playlist.isPlaying && !playlist.isLoading) {
                                    // Clear the timeout reference first
                                    playlist.timeout = null;
                                    playlist._lastTimeoutId = null;
                                    
                                    // Mark as not playing BEFORE stopping to prevent race conditions
                                    playlist.isPlaying = false;
                                    playlist._durationScheduled = false;
                                    
                                    // Ensure BGM is stopped and buffer is destroyed
                                    AudioManager.stopBgm();
                                    
                                    // Clear the buffer reference
                                    playlist.currentPlayingBuffer = null;
                                    
                                    // Wait a moment to ensure buffer is fully destroyed
                                    setTimeout(() => {
                                        // Double-check we're still supposed to continue
                                        if (playlist && !playlist.isLoading && !playlist.isPlaying) {
                                            $gameSystem.launchPlaylistTrack();
                                        }
                                    }, 100);
                                }
                            ...
                            console.warn("Couldn't determine audiofile length:", trackFileName);
                            playlist.isPlaying = false;
                            playlist._durationScheduled = false;
                        }
                    };
                    ...
                    if (playlist.currentPlayingBuffer._totalTime) {
                        // Buffer already loaded, schedule immediately
                        scheduleNext();
                    } else {
                        // Wait for buffer to load
                        playlist.currentPlayingBuffer.addLoadListener(() => {
                            scheduleNext();
                        });
                    }
                } else {
                    console.warn("Failed to create BGM buffer for:", trackFileName);
                    playlist.isPlaying = false;
                    playlist._durationScheduled = false;
                }
            }
        }, 100); // Wait to ensure previous buffer is destroyed
    };
...
// Modified code
    ...
    // Debug: Skip to next track when H key is pressed
    const Origin_Input_onKeyDown = Input._onKeyDown;
    Input._onKeyDown = function(event) {
        Origin_Input_onKeyDown.call(this, event);
        
        // H key is keyCode 72, check for H key press (not when modifiers are held)
        if (event.keyCode === 72 && !event.ctrlKey && !event.altKey && !event.shiftKey) {
            // Only skip if a playlist is active
            if ($gameSystem._playlist) {
                // Use a flag to prevent double-triggering
                if (!$gameSystem._playlist._skipKeyPressed) {
                    $gameSystem._playlist._skipKeyPressed = true;
                    $gameSystem.skipToNextTrack();
                    console.log("Debug: Skipped to next track in playlist");
                    // Reset flag after a short delay
                    setTimeout(() => {
                        if ($gameSystem._playlist) {
                            $gameSystem._playlist._skipKeyPressed = false;
                        }
                    }, 200);
                }
            }

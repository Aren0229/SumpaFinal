...
r88.Casino.Slots = class extends r88.Casino.Game {
   ...
   Reel = class extends Window {
      ...
      #maxSpinTimer = 50;
      #gap = 20;
      #posArray = [995, 870, 745, 635, 515, 410];
      #symbols = ['B2', 'C', 'S', 'B1', 'G', 'BELL'];
      ...
      update() {
         if (this.#spinTimer > 0) {
            ...
            this.spin();
         } else if (!this.#posArray.includes(this.#r1.y)) {
            // Timer expired but not on valid position - snap to nearest
            this.spin();
         }
      }
      ...
      spin() {
         // Continue spinning if timer is still active
         if (this.#spinTimer > 0) {
            ...
            // Timer expired - randomly select a position from the array
            const randomIndex = Math.floor(Math.random() * this.#posArray.length);
            const selectedPos = this.#posArray[randomIndex];
            this.#r1.y = selectedPos;
            // Adjust r2 to maintain proper spacing (half the reel height away)
            this.#r2.y = this.#r1.y + this.#reelHeight;
            if (this.#r2.y >= this.#max) {
               this.#r2.y = this.#r1.y - this.#reelHeight;
            }
         }

         // When timer expires and we're on a valid position, stop and set symbol
         // Only process payout if a spin was actually initiated
         if (this.#spinTimer <= 0 && this.#posArray.includes(this.#r1.y) && this.#parent.#spinInProgress) {
